export type Quadrant = 'KK' | 'KU' | 'UK' | 'UU';
...
export const QUADRANT_MAP: Record<Quadrant, { spo: string; role: string; repl: string; io: string }> = {
...
export function rotatePoint(baseX: number, baseY: number, angleDeg: number) {
...
export function evaluateFaces(quads: Record<number, Quadrant>): FaceRow[] {
...
export function computeCentroid(faces: FaceRow[]) {
...
export function genBasisHash(): string {
  return (
    '0x' +
